  ...
  where
    ...
    flattenType parentIndexNamesAndVars@(unzip -> (parentIndexNames, parentIndexVars)) typeName = do
      ...
      let
        ...
        (fieldsToFlatten, fieldsToKeep) = mconcat . flip map fields $ \field@(_, _, fieldType) -> case fieldType of
          AppT (ConT (Name (OccName "Vector") _)) (ConT (flip elem typesNames -> True)) ->
            ([field], [])
          _ -> ([], [field])

        -- generate some stuff
        Just (typeTitle, _) = find ((== typeName) . snd) types
        flattenedTypeName = suffixName "_flattened" typeName

      fieldsToFlattenVars <- mapM genFieldVar fieldsToFlatten
      fieldsToKeepVars <- mapM genFieldVar fieldsToKeep

      -- find prefix used
      let (takeWhile (/= '_') . nameStr -> fieldPrefix, _, _) = head fieldsToKeep
